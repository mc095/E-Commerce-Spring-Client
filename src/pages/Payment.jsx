...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    // Fetch order summary
    fetch(`${import.meta.env.VITE_API_URL}/api/checkout/${userId}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const applyCoupon = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/coupons/apply?code=${couponCode}`);
  ...
  const confirmPayment = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/checkout/confirm-payment`, {
        method: 'POST',
