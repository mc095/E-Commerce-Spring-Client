  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const similarRes = await fetch(`${import.meta.env.VITE_API_URL}/api/products?category=${encodeURIComponent(data.category)}`);
        if (similarRes.ok) {
  ...
  const addToCart = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/cart/add?userId=${encodeURIComponent(userId)}&productId=${productId}&quantity=1&grams=${grams}&finalPrice=${finalPrice}`,
        {
          method: "POST",
