...
const Cart = () => {
        ...
        const items = await Promise.all(
          cartData.productIds
            ...
            .map(async (productId, index) => {
              try {
                const productRes = await fetch(`${import.meta.env.VITE_API_URL}/api/products/${productId}`);
  ...
  const updateQuantity = async (productId, newQty) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/cart/update?userId=${userId}&productId=${productId}&quantity=${newQty}`, {
        method: 'PUT',
  ...
  const removeItem = async (productId, productName) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/cart/remove?userId=${userId}&productId=${productId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
